...
/** Original Authors from
 * https://github.com/rishidewan33/Advanced-Encryption-Standard-Algorithm/tree/master/src
 * @author Patrick Lu
 * @author Rishi Dewan
 * 
 * main function modified by W Clay Moody for single round and manual entry of
 * line, key, and encrypt/decrypt action. Additional instrumentation provided
 * also.
 */
public class AES {
    ...
    public static void main(String args[]) throws IOException 
    {
        ...
        // AES Example from textbook page 175
        key  = "0f1571c947d9e8590cb7add6af7f6798";
        
        String action = "encrypt";
        String line = "0123456789abcdeffedcba9876543210";
        
        //String action = "decrypt";
        //String line = "FF0B844A0853BF7C6934AB4364148FB9";
        ...
        for (int i = 0; i < numRounds+1; i++) {
            ...
            MatrixToString(aes.subKey(keymatrix, i)));
        }
        System.out.println();
        ...
        if (action.equalsIgnoreCase("encrypt")) {
            ...
            System.out.println("Post AddRndKey0 " + MatrixToString(state) +
            "\n");
            for (int i = 1; i < numRounds; i++) {
                System.out.println("Encryption Round  " + String.format("%d",i));
                aes.subBytes(state); //implements the Sub-Bytes subroutine.
                System.out.println("Post SubBytes:  " + MatrixToString(state));
                aes.shiftRows(state); //implements Shift-Rows subroutine.
                System.out.println("Post ShiftRows: " + MatrixToString(state));
                aes.mixColumns(state);
                System.out.println("Post MixCols:   " + MatrixToString(state));
                aes.addRoundKey(state, aes.subKey(keymatrix, i));
                System.out.println("Post AddRndKey: " + MatrixToString(state) + "\n" );
            }
            System.out.println("Encryption Round 10");
            aes.subBytes(state); //implements the Sub-Bytes subroutine.
            System.out.println("Post SubBytes:  " + MatrixToString(state));
            aes.shiftRows(state); //implements Shift-Rows subroutine.
            System.out.println("Post ShiftRows: " + MatrixToString(state));
            aes.addRoundKey(state, aes.subKey(keymatrix, numRounds));
            System.out.println("Final Cybertext: " + MatrixToString(state));
        }
            ...
            for (int i = numRounds - 1; i > 0; i--) {
                System.out.println("Decryption Round  " +
                String.format("%d",10-i));
                aes.invShiftRows(state);
                System.out.println("Post InvShiftRows:   " + MatrixToString(state));
                aes.invSubBytes(state);
                System.out.println("Post InvSubBytes:    " + MatrixToString(state));
                aes.addRoundKey(state, aes.subKey(keymatrix, i));
                System.out.println("Post InvRoundKeys:   " + MatrixToString(state));
                aes.invMixColumns(state);
                System.out.println("Post InvMixColumns:  " +
            ...
            System.out.println("Final Plaintext: " + MatrixToString(state));
